...
static bool doesHistorySLILCostDominateFrstPss(InstCount OtherPrefixSpillCost,
  ...
  if (ImprovementOnHistory <= RequiredImprovement) {
    if (HistTotalCost - ImprovementOnHistory < 0 && OtherNode->getIsFirstPass()) llvm::errs() << "setting invalid LBC from HistSLIL 1\n";
    ...
    if (OtherNode->GetLocalBestCost() == -1 && OtherNode->getIsFirstPass()) llvm::errs() << "setting invalid LBC from HistSLIL 2\n";
...
bool CostHistEnumTreeNode::chkCostDmntnForTwoPass(EnumTreeNode *Node,
  ...
  if (Node->GetCostLwrBound() >= partialCost_) {
    ...
    if (Node->GetCostLwrBound() == -1 && Node->getIsFirstPass()) errs() << "setting invalid LBC from HistCostDom 1\n";
    Node->SetLocalBestCost(Node->GetCostLwrBound());
    if (Node->GetParent()) {
      if (Node->GetCostLwrBound() == -1 && Node->getIsFirstPass()) errs() << "setting invalid LBC from HistCostDom 2\n";
